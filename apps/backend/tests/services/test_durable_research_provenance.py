...
class DurableResearchProvenanceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_exhausted_work_fails_when_objective_evaluation_has_gaps(self):
        directory = tempfile.TemporaryDirectory()
        self.addCleanup(directory.cleanup)
        repository = SQLiteDurableExecutionRepository(Path(directory.name) / "incomplete.sqlite3")
        await repository.initialize()
        provider = CountingResearchProvider()
        tools = ToolManager()
        tools.register(WebSearchTool(provider))
        durable = DurableExecutionService(repository)
        run = await durable.start(
            objective="Research a market and create and verify a working local prototype.",
            execution_context={}, execution_id="incomplete-run"
        )
        await durable.persist_validated_plan(
            run.execution_id,
            ExecutionPlan(steps=[PlanStep(id=1, action="research market", tool="web_search", input="market", arguments={"query": "market"})]),
        )
        executor = ExecutorService(None, ContextBuilder(), tools, VariableResolver(), PermissionService(), repository)
        await executor.execute_ready_step(run.execution_id, 1, [], ExecutionState(), finalize=False)

        restored = await RecoveryService(SQLiteDurableExecutionRepository(repository.database_path)).prepare_resume(run.execution_id)
        evaluation = ObjectiveEvaluator().evaluate(run.objective, restored.execution_state, [])
        self.assertFalse(evaluation.complete)
        self.assertTrue(await durable.fail_if_finished(run.execution_id))
        self.assertEqual((await durable.prepare_resume(run.execution_id)).status, ExecutionRunStatus.FAILED)
